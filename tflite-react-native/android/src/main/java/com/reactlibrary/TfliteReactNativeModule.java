...
public class TfliteReactNativeModule extends ReactContextBaseJavaModule {
  ...
  private void loadModel(final String modelPath, final String labelsPath, final int numThreads, final Callback callback)
    ...
    // deleteCache(reactContext);
  }

  @ReactMethod
  private void loadModelTwo(final String modelPath, final String labelsPath, final int numThreads,
    ...
    final Interpreter.Options tfliteOptions = new Interpreter.Options();
    tfliteOptions.setNumThreads(numThreads);
    tfLiteNew = new Interpreter(buffer, tfliteOptions);
  ...
  ByteBuffer feedInputTensorImageNew(String path, float mean, float std) throws IOException {
    Tensor tensor = tfLiteNew.getInputTensor(0);
    inputSize = tensor.shape()[1];
    int inputChannels = tensor.shape()[3];
  ...
  private WritableArray parseSSDMobileNet(int numDetections, int numResultsPerClass, float[][][] outputLocations,
    ...
    for (int i = 0; i < numDetections; ++i) {
      ...
      String detectedClass = labels.get(0);
  ...
  private WritableArray parseYOLO(float[][][][] output, int inputSize, int blockSize, int numBoxesPerBlock,
      int numClasses,
    ...
    if (outputTensor.dataType() == DataType.FLOAT32) {
      for (int i = 0; i < outputHeight; ++i) {
        for (int j = 0; j < outputWidth; ++j) {
          ...
          if (outputType.equals("png")) {
            outputArgmax.setPixel(j, i,
                Color.rgb((labelColor >> 16) & 0xFF, (labelColor >> 8) & 0xFF, labelColor & 0xFF));
          } else {
  ...
  @ReactMethod
  private void runSegmentationOnImage(final String path, final float mean, final float std,
  ...
  void initPoseNetNew(Map<Integer, Object> outputMap) {
    if (partsIds.size() == 0) {
      for (int i = 0; i < partNames.length; ++i)
        partsIds.put(partNames[i], i);
    ...
    for (int i = 0; i < tfLiteNew.getOutputTensorCount(); i++) {
      int[] shape = tfLiteNew.getOutputTensor(i).shape();
      float[][][][] output = new float[shape[0]][shape[1]][shape[2]][shape[3]];
      outputMap.put(i, output);
    }
  }
  ...
  @ReactMethod
  private void runPoseNetNew(final String path, final float mean, final float std, final int numResults,
      final float threshold, final int nmsRadius, final Callback callback) throws IOException {
    int localMaximumRadius = 1;
    int outputStride = 16;

    ByteBuffer imgData = feedInputTensorImageNew(path, mean, std);
    Object[] input = new Object[] { imgData };

    Map<Integer, Object> outputMap = new HashMap<>();
    initPoseNetNew(outputMap);
    tfLiteNew.runForMultipleInputsOutputs(input, outputMap);
