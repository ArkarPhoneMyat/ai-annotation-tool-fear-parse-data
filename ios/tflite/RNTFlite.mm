...
//  aiannotation
//
//  Created by Macintosh on 6/22/23.
//
//#define CONTRIB_PATH
#import "RNTFlite.h"
...
RCT_EXPORT_METHOD(loadModelTwo:(NSString *)model_file
                  withLabels:(NSString *)labels_file
                  numThreads:(int)num_threads
                  callback:(RCTResponseSenderBlock)callback)
{
  ...
  model2 = tflite::FlatBufferModel::BuildFromFile([graph_path UTF8String]);
  LOG(INFO) << "Loaded model " << graph_path;
  model2->error_reporter();
  LOG(INFO) << "resolved reporter";
  ...
  if (num_threads != -1) {
    interpreter2->SetNumThreads(num_threads);
  }
...
void feedInputTensor2(uint8_t* in, int* input_size, int image_height, int image_width, int image_channels, float input_mean, float input_std) {
  assert(interpreter2->inputs().size() == 1);
  int input = interpreter2->inputs()[0];
  TfLiteTensor* input_tensor = interpreter2->tensor(input);
  const int input_channels = input_tensor->dims->data[3];
  const int width = input_tensor->dims->data[2];
  const int height = input_tensor->dims->data[1];
  *input_size = width;
  ...
  if (input_tensor->type == kTfLiteUInt8) {
    uint8_t* out = interpreter2->typed_tensor<uint8_t>(input);
    for (int y = 0; y < height; ++y) {
      const int in_y = (y * image_height) / height;
      uint8_t* in_row = in + (in_y * image_width * image_channels);
      uint8_t* out_row = out + (y * width * input_channels);
    ...
    float* out = interpreter2->typed_tensor<float>(input);
    for (int y = 0; y < height; ++y) {
      const int in_y = (y * image_height) / height;
      uint8_t* in_row = in + (in_y * image_width * image_channels);
      float* out_row = out + (y * width * input_channels);
...
void feedInputTensorImage(const NSString* image_path, float input_mean, float input_std, int* input_size, bool is_model_one) {
  int image_channels;
  int image_height;
  int image_width;
  ...
  if(is_model_one){
    ...
    feedInputTensor2(in, input_size, image_height, image_width, image_channels, input_mean, input_std);
  }
}
void feedInputTensorFrame(Frame* frame, float input_mean, float input_std, int* input_size, bool is_model_one) {
  int image_channels;
  int image_height;
  int image_width;
  
  std::vector<uint8_t> image_data = LoadFrame(frame, &image_width, &image_height,&image_channels);
  uint8_t* in = image_data.data();
  if(is_model_one){
    feedInputTensor(in, input_size, image_height, image_width, image_channels, input_mean, input_std);
  }else{
...
void feedInputTensorFrameCamera(Frame* frame, float input_mean, float input_std, int* input_size, int* input_size2) {
  int image_channels;
  int image_height;
  int image_width;
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
    ...
    if(!interpreter2){
      ...
      feedInputTensor2(in, input_size2, image_height, image_width, image_channels, input_mean, input_std);
    }
...
RCT_EXPORT_METHOD(runModelOnImage:(NSString*)image_path
  ...
  int input_size;
  feedInputTensorImage(image_path, input_mean, input_std, &input_size, true);
...
NSMutableArray* parseSSDMobileNet(float threshold, int num_results_per_class) {
  ...
  float* output_locations = interpreter->typed_output_tensor<float>(1);
  float* output_classes = interpreter->typed_output_tensor<float>(3);
  float* output_scores = interpreter->typed_output_tensor<float>(0);
  float* num_detections = interpreter->typed_output_tensor<float>(2);
...
NSMutableArray* parseSSDMobileNet2(float threshold, int num_results_per_class) {
  assert(interpreter2->outputs().size() == 4);
  
  NSMutableArray* results = [NSMutableArray array];
  float* output_locations = interpreter2->typed_output_tensor<float>(1);
  float* output_classes = interpreter2->typed_output_tensor<float>(3);
  float* output_scores = interpreter2->typed_output_tensor<float>(0);
  float* num_detections = interpreter2->typed_output_tensor<float>(2);
...
bool withinNmsRadiusOfCorrespondingPoint(NSMutableArray* poses,
  ...
  for (NSMutableDictionary* pose in poses) {
    ...
    //[NSNumber numberWithInt:keypoint_id]
...
bool withinNmsRadiusOfCorrespondingPointString(NSMutableArray* poses,
                                         float squared_nms_radius,
                                         float y,
                                         float x,
                                         int keypoint_id,
  ...
  for (NSMutableDictionary* pose in poses) {
    NSMutableDictionary* keypoints = pose[@"keypoints"];
    //[NSNumber numberWithInt:keypoint_id]
    NSString *keypoint_id_string = @(keypoint_id).stringValue;
    NSMutableDictionary* correspondingKeypoint = keypoints[keypoint_id_string];
    float _x = [correspondingKeypoint[@"x"] floatValue] * input_size - x;
    float _y = [correspondingKeypoint[@"y"] floatValue] * input_size - y;
    float squaredDistance = _x * _x + _y * _y;
    if (squaredDistance <= squared_nms_radius)
...
NSMutableArray* parsePoseNet2(int num_results, float threshold, int nms_radius, int input_size) {
  initPoseNet();
  
  assert(interpreter2->outputs().size() == 4);
  TfLiteTensor* scores_tensor = interpreter2->tensor(interpreter2->outputs()[0]);
  height = scores_tensor->dims->data[1];
  width = scores_tensor->dims->data[2];
  num_keypoints = scores_tensor->dims->data[3];
  
  float* scores = interpreter2->typed_output_tensor<float>(0);
  float* offsets = interpreter2->typed_output_tensor<float>(1);
  float* displacements_fwd = interpreter2->typed_output_tensor<float>(2);
  float* displacements_bwd = interpreter2->typed_output_tensor<float>(3);
  ...
  while([results count] < num_results && !pq.empty()) {
    ...
    //int key_point_part = [root[@"partId"] intValue];
    //NSString *key_point_part_string = @(key_point_part).stringValue;
    
    if (withinNmsRadiusOfCorrespondingPointString(results, sqared_nms_radius, root_point[0], root_point[1],
                                            [root[@"partId"] intValue], input_size))
      continue;
    ...
    int key_point_part = [root[@"partId"] intValue];
    NSString *key_point_part_string = @(key_point_part).stringValue;
    [keypoints setObject:keypoint forKey:key_point_part_string];
    
    for (int edge = num_edges - 1; edge >= 0; --edge) {
      int source_keypoint_id = [parent_to_child_edges[edge] intValue];
      NSString *source_keypoint_id_string = @(source_keypoint_id).stringValue;
      int target_keypoint_id = [child_to_parent_edges[edge] intValue];
      NSString *target_keypoint_id_string = @(target_keypoint_id).stringValue;
      if (keypoints[source_keypoint_id_string] &&
          !(keypoints[target_keypoint_id_string])) {
        keypoint = traverseToTargetKeypoint(edge, keypoints[source_keypoint_id_string],
                                            target_keypoint_id, scores, offsets, displacements_bwd, input_size);
        [keypoints setObject:keypoint forKey:target_keypoint_id_string];
      }
    }
...
RCT_EXPORT_METHOD(runPoseNetNew:(NSString*)image_path
                  mean:(float)input_mean
                  std:(float)input_std
                  numResults:(int)num_results
                  threshold:(float)threshold
  ...
  image_path = [image_path stringByReplacingOccurrencesOfString:@"file://" withString:@""];
  int input_size;
  feedInputTensorImage(image_path, input_mean, input_std, &input_size, false);
  
  if (interpreter2->Invoke() != kTfLiteOk) {
    NSLog(@"Failed to invoke!");
    callback(@[@"Failed to invoke!"]);
  }
  NSMutableArray* output = parsePoseNet2(num_results, threshold, nms_radius, input_size);
  callback(@[[NSNull null], output]);
}
...
@interface ObjectDetectionProcessPlugin :  NSObject
@end
@implementation ObjectDetectionProcessPlugin;

//-(void)handleInGroupWithCompleteBlock: (void(^)(NSMutableArray * resultArray))completeBlock
//                              {
//
//    NSMutableArray *friendUsers = [[NSMutableArray alloc] init];
//  //NSMutableArray*   results = [[NSMutableArray alloc] init];
//    dispatch_queue_t handleFrame = dispatch_queue_create("handleFrame", NULL);
//    dispatch_async(handleFrame, ^{
...
//      }
//      int input_size;
//      feedInputTensorFrame(frame, 127.5, 127.5, &input_size, true);
//
//      if (interpreter->Invoke() != kTfLiteOk) {
//        NSLog(@"Failed to invoke!");
//      }
//      NSMutableArray*  results = parseSSDMobileNet(0, 5);//
...
static inline id process_frame_plugin(Frame* frame, NSArray* args) {
    int input_size;
    int input_size2;
    int threshold = 0;
    int nms_radius = 20;
  ...
  feedInputTensorFrameCamera(frame, 127.5, 127.5, &input_size, &input_size2);
    if (interpreter->Invoke() != kTfLiteOk) {
      NSLog(@"Failed to invoke!");
    }
  if (interpreter2->Invoke() != kTfLiteOk) {
  ...
  NSMutableArray*   detectBall = parseSSDMobileNet(threshold, num_results);
  NSMutableArray*   detecHoop = parseSSDMobileNet2(threshold, num_results);//
 // NSMutableArray* detectPose = parsePoseNet2(num_results ,threshold , nms_radius, input_size2);
  NSMutableArray *resultFrame = [[NSMutableArray alloc] init];
  NSMutableDictionary* kq = [[NSMutableDictionary alloc] init];
  [kq setObject:detectBall forKey:@"Ball"];
  //[kq setObject:detectPose forKey:@"Pose"];
  [kq setObject:detecHoop forKey:@"Hoop"];
  [resultFrame addObject:kq];
  return resultFrame;
  
}


VISION_EXPORT_FRAME_PROCESSOR(process_frame_plugin)
